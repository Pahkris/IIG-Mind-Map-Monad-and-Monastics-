...
const InfoCard: React.FC<InfoCardProps> = ({ node, onClose, progress, onQuizComplete }) => {
  ...
  return (
    <div className="absolute top-0 right-0 h-full w-full md:w-[500px] bg-void/95 border-l border-blue-900/50 z-[110] overflow-y-auto p-8 shadow-[-20px_0_60px_rgba(0,0,0,0.9)] backdrop-blur-xl transition-all duration-500">
      ...
      {activeDetail && (
        <div className="absolute inset-0 z-[120] bg-black/98 p-12 flex flex-col animate-in fade-in slide-in-from-right-10 duration-500">
          <button 
            onClick={() => setActiveDetail(null)} 
            className="self-end p-2 text-gray-500 hover:text-white hover:bg-white/5 rounded-full transition"
          >
            <ICONS.Close />
          </button>
          ...
          <div className="flex-1 mt-4">
            ...
            <h3 className="text-4xl font-mystic text-white mb-6 tracking-wider leading-tight border-b border-white/5 pb-4">
              {activeDetail.title}
            </h3>
            
            <div className="space-y-8 overflow-y-auto pr-2 max-h-[70vh] custom-scrollbar">
              <div>
                <h4 className="text-[11px] font-bold text-blue-400 uppercase tracking-widest mb-4">The Theory</h4>
              ...
              <div className="pt-8 border-t border-white/5">
                ...
                <div className="space-y-4">
                  {activeDetail.exercises.map((ex, i) => (
                    <div key={i} className="group flex items-start gap-5 bg-white/5 p-6 rounded-2xl border border-white/10 hover:border-orange-500/40 hover:bg-orange-500/5 transition duration-300">
                      <div className="w-8 h-8 rounded-full bg-orange-500/20 text-orange-500 flex items-center justify-center font-bold text-xs group-hover:bg-orange-500 group-hover:text-black transition-colors">
                        {i + 1}
                      </div>
          ...
          <button 
            onClick={() => setActiveDetail(null)}
            className="mt-10 w-full py-5 bg-blue-600 rounded-2xl font-bold uppercase tracking-[0.2em] text-[11px] hover:bg-blue-500 transition shadow-[0_10px_30px_rgba(37,99,235,0.4)] transform hover:-translate-y-1"
          >
            Return to Core Module
          </button>
        </div>
      )}
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            <ul className="space-y-4">
              {node.content.map((item, i) => (
                <li 
                  key={i} 
                  onClick={() => node.detailedInfo?.[i] && setActiveDetail(node.detailedInfo[i])}
                  className={`flex gap-5 items-start bg-black/40 p-5 rounded-2xl border border-white/5 transition-all duration-300 group ${node.detailedInfo?.[i] ? 'cursor-pointer hover:border-blue-500/60 hover:bg-blue-900/20 hover:translate-x-1' : ''}`}
                >
                  <div className="w-6 h-6 rounded-lg bg-blue-500/10 text-blue-400 flex items-center justify-center font-bold text-xs group-hover:bg-blue-500 group-hover:text-white transition-colors">
                    {i + 1}
                  </div>
